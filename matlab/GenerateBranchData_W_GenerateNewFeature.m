...
close all
clc
rootfolder = 'E:\AutomatedTracing\TraceProofreading\';
addpath([rootfolder,'common_functions\NeuronTracerV20'])
user = ['AS';'RG';'JC'];
numuser = 3;

for num = 1:6
    % Create Scenarios
    parameter_file_path=[rootfolder,'common_functions\Parameter Files\Parameters_L6.txt'];
    Parameters=Read_Parameter_File(parameter_file_path);
    G = load([rootfolder,'data\traces\L1\',num2str(num),'_L6_',user(numuser,:),'.mat']);
    AM = G.AM;
    r = G.r;
    R = G.R;
...
%     end
    save([rootfolder,'data\traces\L1\',num2str(num),'_L6_',user(numuser,:),'_withALLClusters1_NewFeatures.mat']);
end
...
%     D1 = load(['E:\AutomatedTracing\Data\Traces\L1_org\',num2str(n),'_L6_',user(numuser,:),'_withALLClusters1.mat']);
    D2 = load([rootfolder,'data\traces\L1\',num2str(n),'_L6_',user(numuser,:),'_withALLClusters1_NewFeatures.mat']);
    D1 = D2;
    ...
    GG = load([rootfolder,'data\traces\L1_org\',num2str(n),'_L6_',user(numuser,:),'.mat']);
...
path = [rootfolder,'data\datafeed\IMonce_100_scen_NEW_Inv_FEATURES_User=',user(numuser,:),'.mat'];
disp(path)
