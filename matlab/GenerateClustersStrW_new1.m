...
function [AMlbl,r,R,ClustersStr]=GenerateClustersStrW_new1(Orig,AMlbl,r,R,Parameters,varargin)


% To get All the scenarios
Parameters.BranchMerger.NBestScenarios = inf;
...
[~,~,R,I_F]=Optimize_Trace_R(Orig,AMlbl,r,R,0,1,0,Parameters.Optimization.PointsPerVoxel,1,Parameters.Optimization.TraceStiffness,Parameters.Optimization.RadiusStiffness,0,0,0,myClass);

if myClass.getFlag()==1
    return;
end

ClustersStr=struct('end_point_ind',{},'end_point_label',{},'end_point_r',{},'end_point_n',{},'scenarios',{},'cost_components',{},'best_merger',{},'alpha',{},'w',{},'weights',{},'confidence',{});
...
for i=1:ClusterN
    ...
    for k=1:length(TipLabels)
        ...
        if ClusterV{i}(k)<=length(L)
            ...
            temp=r(tip1V0(TipLabels(k)),:)-r(tip1Vend(TipLabels(k)),:);
            ClustersStr(i).end_point_n(k,:)=temp./sum(temp.^2).^0.5;
        else
            tempV=tip2V{TipLabels(k)};
            ClustersStr(i).end_point_ind(k)=tip2V0(TipLabels(k));
            temp=r(tip2V0(TipLabels(k)),:)-r(tip2Vend(TipLabels(k)),:);
            ClustersStr(i).end_point_n(k,:)=temp./sum(temp.^2).^0.5;
        end
